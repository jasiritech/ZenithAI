...
{Colors.RESET}""")
    ...
    Display._box([
        ...
        f"  {Colors.CYAN}[3]{Colors.RESET} OpenAI-Compatible / Custom {Colors.DIM}─ APInex, DeepSeek, Ollama, WusRouter 🚀{Colors.RESET}",
    ], color=Colors.CYAN, style="rounded")
    print()
    
    provider_choice = input(f"  {Colors.YELLOW}  ▸ Choose [1/2/3] (default: 2 for Groq): {Colors.RESET}").strip()
    base_url = None
    ...
    if resume_session:
        ...
        if session_data:
            ...
            return api_key, target, goal, model, max_iter, None, None, None, None, resume_session, None, provider, False
        else:
    ...
    if provider == "groq":
        ...
        default_model = "free/gemini-3.8-flash" if "apinex" in str(base_url) else "gpt-4o-mini"
        print()
        Display._box([
            f"{Colors.BOLD}🧠 AI MODEL FOR CUSTOM PROVIDER{Colors.RESET}",
            f"",
            f"  {Colors.DIM}Examples: free/gemini-3.8-flash, wusrouter/kimi-k2.5, deepseek-chat{Colors.RESET}",
            f"  {Colors.DIM}Default: {default_model}{Colors.RESET}",
        ], color=Colors.CYAN, style="rounded")
        print()
        m_input = input(f"  {Colors.YELLOW}  ▸ Model name [{default_model}]: {Colors.RESET}").strip()
        model = m_input if m_input else default_model
        print(f"  {Colors.GREEN}  [✓] Selected model: {model}{Colors.RESET}")
    else:
        print()
        Display._box([
    ...
    # ═══════════════════════════════════════
    # STEP 9: SCANNING ENGINE MODE
    # ═══════════════════════════════════════
    print()
    Display._box([
        f"{Colors.BOLD}⚡ SCANNING ENGINE MODE{Colors.RESET}",
        f"",
        f"  {Colors.GREEN}[1]{Colors.RESET} Autonomous AI Loop  {Colors.DIM}─ Dynamic action-by-action execution{Colors.RESET}",
        f"  {Colors.MAGENTA}[2]{Colors.RESET} Multi-Agent Swarm   {Colors.DIM}─ 5-Agent Collaborative Pipeline (Planner, Recon, Web, Exploit, Reporter) ⭐{Colors.RESET}",
    ], color=Colors.CYAN, style="rounded")
    print()
    engine_choice = input(f"  {Colors.YELLOW}  ▸ Choose mode [1/2] (default: 1): {Colors.RESET}").strip()
    run_pipeline = (engine_choice == "2")

    # ═══════════════════════════════════════
    # CONFIRM & LAUNCH
    # ═══════════════════════════════════════
    if provider == "custom":
        model_display = f"{Colors.CYAN}Custom ({model}) 🚀{Colors.RESET}"
    elif model == "groq" or (api_key and api_key.startswith("gsk_")):
    ...
    mode_display = f"{Colors.MAGENTA}Multi-Agent Swarm (5 Agents){Colors.RESET}" if run_pipeline else f"{Colors.GREEN}Autonomous AI Loop{Colors.RESET}"

    print()
    Display._box([
        f"{Colors.BOLD}🚀 READY TO LAUNCH{Colors.RESET}",
        f"",
        f"  Engine   {mode_display}",
    ...
    return api_key, target, goal, model, max_iterations, profile_name, proxy_config, notify_config, sudo_password, None, base_url, provider, run_pipeline
...
Profiles: quick, full, stealth, web, network, api, recon-only
    ...
    parser.add_argument('-m', '--model', default='flash',
                       help='AI model: pro, flash, groq, or custom name (e.g. free/gemini-3.8-flash)')
    parser.add_argument('--provider', choices=['gemini', 'groq', 'custom', 'openai'], default=None,
                       help='AI provider (default: auto-detect from key or base-url)')
    parser.add_argument('--base-url', help='Custom OpenAI-compatible base URL (e.g. https://api.apinex.bond/v1)')
    parser.add_argument('--pipeline', '--agentic', action='store_true', dest='pipeline',
                       help='Run 5-agent collaborative swarm pipeline (Planner, Recon, Web, Exploit, Reporter)')
...
def main():
    ...
    if args.config:
        ...
        api_key = config.get('api_key', os.environ.get('OPENAI_API_KEY', '') or os.environ.get('GEMINI_API_KEY', ''))
        base_url = config.get('base_url', config.get('custom_api_base', None))
        provider = config.get('provider', None)
        run_pipeline = config.get('pipeline', False) or getattr(args, 'pipeline', False)
    ...
    # ═══════════════════════════════════════
        ...
        run_pipeline = getattr(args, 'pipeline', False)
        api_key = args.api_key or os.environ.get('OPENAI_API_KEY', '') or os.environ.get('GROQ_API_KEY', '') or os.environ.get('GEMINI_API_KEY', '')
        if not api_key:
            print(f"  {Colors.CYAN}[1] Gemini  [2] Groq (recommended)  [3] Custom / APInex{Colors.RESET}")
            p = input("  Provider [1/2/3]: ").strip()
            if p == "1":
                provider = "gemini"
                api_key = input("  Enter Gemini API Key: ").strip()
            elif p == "3":
                provider = "custom"
                b = input("  Enter Base URL [https://api.apinex.bond/v1]: ").strip()
                base_url = b if b else "https://api.apinex.bond/v1"
    ...
    # ═══════════════════════════════════════
        ...
        base_url = result[10] if len(result) > 10 else None
        provider = result[11] if len(result) > 11 else None
        run_pipeline = result[12] if len(result) > 12 else False
        output_dir = None
    ...
    if not api_key:
        ...
        print(f"  {Colors.DIM}Set GROQ_API_KEY, GEMINI_API_KEY, or OPENAI_API_KEY env var, or use -k flag{Colors.RESET}")
        sys.exit(1)
    ...
    if run_pipeline:
        try:
            ...
            print(f"\n  {Colors.YELLOW}[!] Pipeline scan interrupted by user.{Colors.RESET}")
        except Exception as e:
            print(f"\n  {Colors.RED}[!] Fatal pipeline error: {e}{Colors.RESET}")
            import traceback
            traceback.print_exc()
            sys.exit(1)
    else:
