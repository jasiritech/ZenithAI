...
class AIBrain:
    ...
    def __init__(self, api_key, model_choice="flash", base_url=None, provider=None):
        """
        Initialize AI Brain with automatic provider detection and model fallback.
        
        Args:
            api_key: API key (Gemini: AIza..., Groq: gsk_..., or custom sk-...)
            model_choice: 'pro' for deep thinking, 'flash' for speed, 'groq', or any model name
            base_url: Optional base URL for OpenAI-compatible providers (APInex, DeepSeek, Ollama, etc.)
            provider: Optional explicit provider name ('gemini', 'groq', 'custom', 'openai')
        """
        ...
        # Detect provider
        if provider in ("custom", "openai", "openai_compatible") or base_url:
            self.provider = "custom"
            self._init_custom(api_key, model_choice, base_url)
        elif api_key.startswith("gsk_") or model_choice == "groq" or provider == "groq":
            self.provider = "groq"
    ...
    def _init_custom(self, api_key, model_choice, base_url):
        """Initialize custom OpenAI-compatible provider (APInex, DeepSeek, Ollama, OpenRouter, etc.)."""
        ...
        if model_choice and model_choice not in ("flash", "pro", "groq"):
            self.model_name = model_choice
        else:
            self.model_name = "free/gemini-3.8-flash" if "apinex" in url else "gpt-4o-mini"
            
        print(f"    [✓] AI Brain initialized: OpenAI-Compatible Provider")
...
"""
    ...
    def think(self, target, goal=None, knowledge_base=None, last_command="", last_output="", phase="recon"):
        ...
        if goal is None and knowledge_base is None:
        ...
        try:
            ...
            if self.provider == "custom":
                raw_text = self._call_custom(prompt)
    ...
    def _call_custom(self, prompt, system_prompt=None, max_tokens=2048):
        ...
        system_content = system_prompt or "You are a pentesting AI assistant. You MUST respond with ONLY a single valid JSON object. No markdown, no explanations, no code fences. Just raw JSON."
    ...
    def query(self, prompt, system_prompt=None, max_tokens=2048):
        ...
        self.call_count += 1
        if self.provider == "custom":
            return self._call_custom(prompt, system_prompt=system_prompt, max_tokens=max_tokens)
        elif self.provider == "groq":
